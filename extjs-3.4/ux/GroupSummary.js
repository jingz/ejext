    ...
    renderSummary : function(o, cs, g){
        ...
        for(var i = 0, len = cs.length; i < len; i++){
            ...
            if(cf.summaryType || cf.summaryRenderer){
                p.value = (cf.summaryRenderer || c.renderer)(o.data[c.name], p, o, cf);
            }else{
                p.value = '';
            }
        ...
        // MOD 
        if(this.grid.renderMoreSummary){

            return this.rowTpl.apply({
                tstyle: 'width:'+this.view.getTotalWidth()+';',
                cells: buf.join('')
            }) + this.rowTpl.apply({
               tstyle: 'width:'+this.view.getTotalWidth()+';' + "text-align: right; color: #333; font-weight: bold;",
               cells: [
                        this.cellTpl.apply({value: this.grid.renderMoreSummary(g, o)})
                      ].join('')
            });
